...
{
  struct stat st;
  char *path = nodotfile (name);
	int retval = stat(path, &st);
	caml_stat_free(path);	
	return Val_bool(retval == 0);
}
...
{
	char *path1 = nodotfile (oldname);
	char *path2 = nodotfile (newname);
  if (rename(path1, path2) != 0)
    caml_sys_error(NO_ARG);
	caml_stat_free(path1);	
	caml_stat_free(path2);	
	return Val_unit;
}
